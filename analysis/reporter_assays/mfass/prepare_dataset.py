#!/usr/bin/env python3
"""prepare mfass dataset for scoring, outputs h5 with one-hot seqs + metadata"""
import argparse
import sys
...
default_fasta = script_dir.parent / "vex_seq" / "hg19.fa"
...
def load_snv_data(data_dir):
    snv_path = data_dir / "snv_data_clean.txt"
    assert snv_path.exists(), f"missing: {snv_path}"

    df = pd.read_csv(snv_path, sep="\t")

    # keep mutant rows only
    df = df[df["category"] == "mutant"].copy()

    df = df.dropna(subset=["snp_position", "ref_allele", "alt_allele"])

    df["CHROM"] = df["chr"].astype(str)
    df["POS"] = df["snp_position"].astype(int)
    df["REF"] = df["ref_allele"].astype(str)
    df["ALT"] = df["alt_allele"].astype(str)

    return df.reset_index(drop=True)


def validate_spans(df):
    # mfass spans: intron1 + exon + intron2
    cols = ["start", "end", "exon_len", "intron1_len", "intron2_len"]
    assert all(c in df.columns for c in cols), f"missing columns: {cols}"

    s = pd.to_numeric(df["start"], errors="coerce")
    e = pd.to_numeric(df["end"], errors="coerce")
    ex = pd.to_numeric(df["exon_len"], errors="coerce")
    i1 = pd.to_numeric(df["intron1_len"], errors="coerce")
    i2 = pd.to_numeric(df["intron2_len"], errors="coerce")

    span0 = e - s
    span1 = span0 + 1
    sum_all = i1 + ex + i2

    ok_exon = (span0 == ex) | (span1 == ex)
    ok_sum = (span0 == sum_all) | (span1 == sum_all)
    bad = ~(ok_exon | ok_sum) | span0.isna() | ex.isna()

    assert not bad.any(), f"{bad.sum()} rows fail length sanity checks"


def add_orientation(df, fasta):
    # mfass seqs may be forward or revcomp
    orientations = []
    for row in tqdm(df.itertuples(index=False), total=len(df), desc="checking orientation"):
        chrom = str(row.CHROM)
        seq = fasta.fetch(chrom, int(row.start) - 1, int(row.end)).upper()
        nat = str(row.natural_seq).upper()
        if seq == nat:
            orientations.append("forward")
        elif seq == revcomp(nat):
            orientations.append("revcomp")
        else:
            orientations.append("mismatch")

    out = df.copy()
    out["natural_seq_orientation"] = orientations
    mismatch = out["natural_seq_orientation"].eq("mismatch").sum()
    assert mismatch == 0, f"{mismatch} rows have natural_seq mismatches"
    return out
...
def build_sequences(df, builder):
    required = {"CHROM", "strand", "POS", "REF", "ALT", "intron1_len", "exon_len", "intron2_len", "start", "end"}
    ...
    for row_idx, r in enumerate(tqdm(df.itertuples(index=False), total=len(df), desc="building sequences")):
        ...
        ref = str(r.REF)
        alt = str(r.ALT)
        intron1 = int(r.intron1_len)
        exon_len = int(r.exon_len)
        intron2 = int(r.intron2_len)
        start1 = int(r.start)
        end1 = int(r.end)

        # exon boundaries from intron offsets
        exon_start_1b = start1 + intron1
        exon_end_1b = exon_start_1b + exon_len - 1
        if exon_end_1b != end1 - intron2:
            skipped.append((row_idx, f"exon bounds disagree for {chrom}:{start1}-{end1}"))
            continue
        ...
        meta["exon_start"].append(exon_start_1b)
        meta["exon_end"].append(exon_end_1b)

        # v2_dpsi may be missing
        dpsi = r.v2_dpsi if hasattr(r, "v2_dpsi") and pd.notna(r.v2_dpsi) else np.nan
        meta["delta_psi"].append(float(dpsi))

        meta["exon_id"].append(str(r.exon) if hasattr(r, "exon") else "")
        meta["snp_position_type"].append(str(r.snp_pos) if hasattr(r, "snp_pos") else "")
...
def save_h5(seqs, meta, path, seq_len):
    ...
    with h5py.File(path, "w") as f:
        ...
        meta_grp.create_dataset("exon_id", data=np.array(meta["exon_id"], dtype="S64"))
        meta_grp.create_dataset("snp_position_type", data=np.array(meta["snp_position_type"], dtype="S16"))

        # attributes
        f.attrs["n_variants"] = n
        f.attrs["seq_len"] = seq_len
        f.attrs["description"] = "MFASS dataset - multiplex functional assay splice variants"

    n_labeled = np.sum(~np.isnan(meta["delta_psi"]))
    print(f"wrote {path} ({n:,} variants, {n_labeled:,} with delta-PSI labels, {seq_len}bp sequences)")


def main():
    ap = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ap.add_argument("--data-dir", type=Path, default=default_data_dir,
                    help="directory containing snv_data_clean.txt")
    ...
    ap.add_argument("--out-h5", type=Path, default=default_data_dir / "mfass.h5",
    ...
    assert args.fasta.exists(), f"missing fasta: {args.fasta}"

    print("loading raw data...")
    df = load_snv_data(args.data_dir)
    validate_spans(df)
    print(f"  {len(df):,} variants after filtering")

    print("checking sequence orientation...")
    fasta = pysam.FastaFile(str(args.fasta))
    df_oriented = add_orientation(df, fasta)

    print("building sequences...")
    builder = WindowBuilder(fasta, args.window_half)
    seqs, meta, skipped = build_sequences(df_oriented, builder)
    fasta.close()
