...
def load_predictions(pred_dir):
    ...
    return truth, preds, any_df[["chrom", "pos", "strand"]]


def load_shared_sites(matrix_path):
    """find splice sites valid in all samples from splicing matrix, filtered to test chroms"""
...
def main():
    ...
    if args.all:
        ...
        print(f"pooling {len(pred_dirs)} individuals", flush=True)

        all_truth = {"acceptor": [], "donor": [], "ssu": []}
        all_preds = {}

        for pred_dir in pred_dirs:
            truth, preds, _ = load_predictions(pred_dir)
            for k in all_truth:
                all_truth[k].append(truth[k])
            for model, p in preds.items():
                if model not in all_preds:
                    all_preds[model] = {k: [] for k in p}
                for k, v in p.items():
                    all_preds[model][k].append(v)
        ...
        results = compute_metrics(truth, preds, skip_binned=skip_binned, skip_cls=skip_cls, splice_combined=args.splice_combined, regression_only=args.regression_only)
        ...
        skip_cls = args.no_cls or args.regression_only
        skip_binned = args.no_binned and not args.regression_only
        results = compute_metrics(truth, preds, skip_binned=skip_binned, skip_cls=skip_cls, splice_combined=args.splice_combined, regression_only=args.regression_only, valid_all_mask=shared_mask)
        with open(output, 'w') as f:
            json.dump({"overall": results}, f, indent=2, default=float)
...
def compute_metrics(truth, preds, skip_binned=False, skip_cls=False, splice_combined=False, regression_only=False, valid_all_mask=None):
    """compute classification and regression metrics

    splice_combined: use max(acc,don) for models with both outputs (pangolin data)
    regression_only: only compute regression on valid-ssu splice sites
    valid_all_mask: bool array, positions matching sites valid in all samples
    """
    ...
    # binned metrics
    if not skip_binned:
        results["binned"] = compute_binned(is_acc, is_don, ssu, masks, neither_idx, preds, regression_only=regression_only)

    return results


def compute_binned(is_acc, is_don, ssu, masks, neither_idx, preds, regression_only=False):
